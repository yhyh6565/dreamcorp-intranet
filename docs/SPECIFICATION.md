...
3. **Phase 3 - 가짜 터미널 로그** (10~17초)
   - 정상 터미널 로그 먼저 표시
   - 비정상 로그 (빨간색):
     ```
     > SYSTEM ALERT: Unauthorized Access Detected.
     > User: Unknown
     > Location: Tracking... [완료]
     > Camera: ON
     > Microphone: ON
     ```
   - REC ● 표시 (우상단)
   - "Tracking..." 7초간 깜빡임 후 "[완료]" 표시
...
**이스터에그 시퀀스:**
1. 뒤로가기 버튼 사라짐
2. 유니코드 이스케이프 텍스트 타이핑 시작:
   ```
   \uc6b0\ub9ac \ubaa8\ub450\ub294 \ud55c\ub0b1 \uc774\uc57c\uae30\uc5d0 \ubd88\uacfc\ud558\ub2e4 \uc704\ub300\ud558\uc2e0 \uc774\ub984\ub2d8
   ```
   (= "우리 모두는 한낱 이야기에 불과하다 위대하신 이름님")
3. 원본 내용 아래에 타이핑되며 밀어내기 효과
4. 자동 스크롤
5. 10초 후 대시보드로 리다이렉트
6. **사용자 이름이 "■■■"로 변경됨**
7. 해당 세션 내 스팸 쪽지 삭제
...
| # | 위치 | 담당자 | 대상 | 상태 |
|---|------|--------|------|------|
| 1 | 본관 11F D조 | 박민성 주임 | 안녕 교통정보 (Qterw-E-63) | 주의 |
| 2 | 별관 3F 302호 (김솔음 전용) | 김솔음 주임 | 양자택일 (Qterw-F-2073) | 안정 |
| 3 | 별관 3F 격리실 | [현재 사용자] | 양자택일/속삭이는 벽 | 안정 |
| 4 | 본관 14F C조 라운지 | 강■■ 팀장 | Qterw-S-01 [데이터 말소] | 접근 금지 |
| 5 | B1F 창고 | 한■■ 대리 | 식별 불가 (The Thing) | 동면 중 |
| 6 | 본관 15F 1504호 | 조■■ 주임 | Qterw-?-??? | 동면 중 |
...
- 로그아웃 시 스팸 삭제 플래그 초기화 (재로그인 시 스팸 쪽지 복구)
...
| # | 트리거 | 효과 | 결과 |
|---|--------|------|------|
| 1 | 3번째 공지 5초 후 | 점프스케어 (타이핑 + 터미널) | 대시보드로 이동 |
| 2 | 스팸 쪽지 스크롤 후 뒤로가기 | 유니코드 재타이핑 | 이름 손상 + 쪽지 삭제 |
